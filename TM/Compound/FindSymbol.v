...
Section FindSymbol.
  ...
  Definition FindSymbol : { M : mTM sig 1 & states M -> FinType (EqType bool)} :=
    If (move_to_symbol f L)
       (mono_Nop _ true)
       (Move _ R tt ;; move_to_symbol f R).
  ...
  Proof.
    ...
    - unfold FindSymbol. eapply If_Realise.
      + eapply move_to_symbol_l_Realise.
    ...
    - hnf. intros tin (yout, tout). intros H. hnf in H.
      ...
      + destruct H as (t1&H1&->&->). unfold to_symbol. now rewrite <- H1.
      + destruct H as(t1&H1&((b&t2)&H2&H3)); hnf in *. destruct b. destruct H2 as (_&H2).
        unfold to_symbol. rewrite <- H1. cbn. rewrite H2 in *; clear H2. destruct_tapes. now cbn in *.
  Qed.
