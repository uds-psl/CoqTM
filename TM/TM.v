...
we are on the right extremity of a non-empty tape (right overflow). *)
  ...
  Section CannonicalRelation1.
    Variable (n : nat).
    Variable (F : finType).
    Variable (pM : { M : mTM n & states  M -> F }).

    Definition R_cannonical : Rel (tapes n) (F * tapes n) :=
      fun t1 '(y, t2) =>
        exists outc k, loopM (M := projT1 pM) k (initc (projT1 pM) t1) = Some outc /\
                  t2 = ctapes outc /\ projT2 pM (cstate outc) = y.

    Lemma WRealise_R_mTM :
      pM ⊫ R_cannonical.
    Proof. hnf. firstorder eauto. Qed.

    Lemma R_cannonical_functional : functional R_cannonical.
    Proof.
      hnf. intros x (y1&z1) (y2&z2) (c1&k1&H1&->&H1') (c2&k2&H2&->&H2').
      pose proof loop_functional H1 H2 as ->. congruence.
    Qed.

  End CannonicalRelation1.

  Section CannonicalRelation2.
    Variable (n : nat).
    Variable (M : mTM n).

    Definition T_cannonical : Rel (tapes n) nat :=
      fun t k => exists outc, loopM (M := M) k (initc M t) = Some outc.

    Lemma T_cannonical_TerminatesIn :
      M ↓ T_cannonical.
    Proof. firstorder. Qed.

  End CannonicalRelation2.
