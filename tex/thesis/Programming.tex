...
\todo{Matches and Constructors for list types.  The machines may be to complicated, so maybe leave the definitions out and only state the lemmas}
...
$length : \List(X) \to \Nat$.  Again, we use the technique to extend alphabets of Section~\ref{sec:extend-alphabet}.

\subsubsection{$\MS{Nth}$}
\label{sec:Nth}

Let $X$ be encodable on $\Sigma_X$.  We implement a machine $\MS{Nth} : \TM_{\Sigma^+}^4$.  We assume retractions
$f_{\List(X)} : \Sigma_{\List(X)} \hookrightarrow \Sigma$, $f_{\Option(X)} : \Sigma_{\Option(X)} \hookrightarrow \Sigma$, and
$f_\Nat : \Sigma_\Nat \to \Sigma$.  Now there are two ways to encode $X$ on $\Sigma$, via $RetrOpt \circ f_{\Option(X)}$ and
$RetrList \circ f_{\List(X)}$.
