...
\begin{definition}[Functional Computation Relation]
  \label{def:Computes_Rel}
  Let $f : X \to Y$, where $X$ and $Y$ are encodable on $\Sigma$.  Then $FunRel(f) \subseteq \Tape_{\Sigma^+}^{3+n} \times \Tape_{\Sigma^+}^{3+n}$ is defined as:
  \begin{alignat*}{2}
...
Note that this definition can be generalised to unary or functions with higher arity.
...
\end{align*}
with $\MS{Loop} := \While(\MS{Step})$.  Note that the step-machine has then only ``access'' to the copies of $x$ and $y$, but not to the ``original''
on $t[0]$ and $t[1]$; $\MS{Step}$ has two tapes less than $M$.  The copy of $x$ and $y$ is on the $0$st and $1$st tape of $\MS{Step}$, the output is
on tape $2$, all further tapes are internal tapes.  After the loop, all tapes that $\MS{Step}$ did not reset, including the tapes to contain the
copies of $x$ and $y$ and additional tapes, are reseted by $M$.
...
possible.  Then we can define the correctness relation $LoopRel$ of $\MS{Loop}$ and prove $\MS{Loop} \Realise LoopRel$.
...
\subsection{Folding of Sum Functions}
\label{sec:SumTM}
