...
\begin{lemma}[Composition and encoding mapping][Encode_map_comp]
  ...
  \[ encodeMap~(encodeMap~f~encode_\Sigma)~g~(x) = encodeMap~encode_\Sigma~(f \circ g)~x. \]
\end{lemma}
...
\begin{definition}[Basic encodings]
  ...
  ~
  \begin{enumerate}
  \coqitem[Encode_unit] We encode $\Unit$ on the empty alphabet $\False$: $encode~\unit := \nil$
...
\begin{lemma}[$surject$ and value-containment][contains_translate_tau]
  ...
  $t:\Tape_{\Tau^+}$
  \[
    mapTape~(surject~(f \circ RetrPlus)~\MS{UNKNOWN})~t \simeq x ~\iff~ t \simeq_f x
  \]
\end{lemma}
Note that the encoding on the right side is mapped with the retraction $f$.

We write $\LiftAlphabet{M}{f}$ for $\LiftAlphabet{M}{(f \circ RetrPlus, \MS{UNKNOWN})}$.  Note that we also use the $\Uparrow$ notation for the tapes-lift, but it is
...
\label{sec:match-nat}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.MatchNat}%
...
like:
\begin{align*}
  & mapTapes~\left(surject~(f \circ RetrPlus)~\MS{UNKNOWN}\right)~t = \\
  & mapTapes~\left(surject~(f \circ RetrPlus)~\MS{UNKNOWN}\right)~t'
\end{align*}
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.MatchSum}%
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.MatchPair}%
...
\label{sec:MatchList}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.MatchList}%
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.MatchFin}%
...
The tape assignment is visualised in Figure~\ref{fig:tapes-Nth}.
