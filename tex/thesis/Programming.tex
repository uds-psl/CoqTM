...
Using the machine class $\MS{WriteString}$ from Section~\ref{sec:WriteString}, it is possible to define a machine
$\MS{WriteValue}~x : \TM_{\Sigma^+}^1$, where $x:X$ is encodable over $\Sigma$, that writes the value $x$ on a right tape.
\begin{definition}[$\MS{WriteValue}$]
  \label{def:WriteValue}
  Let $x:X$ and $X$ encodable over $\Sigma$.
  \[
    \MS{WriteValue}(x) := \MS{WriteString}~L~\left(\MS{STOP} \cons \rev{(encode(x))}\right) \Seq \MS{Write}(\MS{START}).
  \]
\end{definition}
The stop symbol and the encoding are written on the tape, from right to left in reversed order.  After that, the machine writes the start symbol on
the tape.  This step is done separately, because after the execution of $\MS{WriteString}$, the head of the tape is one step left to the last written
symbol.
\begin{lemma}[Correctness of $\MS{WriteValue}$]
  \label{lem:WriteValue_Sem}
  Let $x:X$.
  \[
    \MS{WriteValue}(x) \RealiseIn{5 + 3 \cdot \size{x}} WriteValueRel(x)
  \]
  \[
    WriteValueRel(x) := \lambda t~t'. isRight(t[0]) \rightarrow t'[0] \simeq x.
  \]
\end{lemma}
...
\end{align*}
with $\MS{Loop} := \While(\MS{Step})$.  Note that the $\MS{Step}$ and $\MS{Loop}$ have only ``access'' to the copies of $x$ and $y$, but not to the
``original'' on $t[0]$ and $t[1]$ of $M$.  This also means that $\MS{Step}$ has two tapes less than $M$.  The copy of $x$ and $y$ is on the $0$st and
$1$st tape of $\MS{Step}$, the output is on tape $2$, all further tapes are internal tapes.  After the loop, all tapes that $\MS{Step}$ did not reset,
including the tapes to contain the copies of $x$ and $y$ and additional tapes, are reseted by $M$.
...
relations $StepT$ and $LoopT$, and show $\MS{Step} \TerminatesIn StepT$, and after that $\MS{Loop} \TerminatesIn LoopT$.
...
We want to implement the functions $add,mult:\Nat\to\Nat$, according to Definition~\ref{def:Computes2_Rel}.  First, we have to define

\subsection{Mapping of Sum Functions}
\label{sec:SumTM}

Let $f : X \to Z$ and $g : Y \to Z$.  Then we can define the canonical function $f+g : X+Y \to Z$ with
\begin{alignat*}{2}
  (f+g)&(\inl x)&~:=~& f(x) \\
  (f+g)&(\inr y)&~:=~& g(y).
\end{alignat*}

We want to define an operator that takes machines $M_1, M_2$ that compute $f$ and $g$, and yields a machine that computes the unary function $f+g$.
