\chapter{Generalised Register Machines}
...
When we speak of ``programming'' Turing machines, this means that instead of using primitive machines (like $\MS{DoAct}$) we only use machines that
directly change the value of a tape (like $\MS{ConstrS}$, which increases a number).  Using the definition of value-containment, we can also formalise
a ``callee-saving'' convention for computation of functions.  Furthermore, we show a general pattern how to program and verify Turing machines, and
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.Code}%
...
\begin{lemma}[Composition and encoding mapping][Encode_map_comp]
  ...
  $g : \Tau \hookrightarrow \Delta$ be retractions.  Then there is (extensionally) only one canonical way how $X$ can be encoded on $\Delta$, i.e.:
  \[
    encodeMap~(encodeMap~f~encode_\Sigma)~g~x = encodeMap~encode_\Sigma~(g \circ f)~x
  \]
\end{lemma}

For every following type $X$ we define an alphabet $\Sigma_X$ to encode $X$ on.  If $X$ is an type constructor and $Y$ is encoded on $\Sigma_Y$, we
define $\Sigma_{X(Y)}$ as the alphabet for $X(Y)$:
\begin{definition}[Basic encodings]
  \label{def:basic-encodings}
  ~
  \begin{enumerate}
  \coqitem[Encode_unit] We encode $\Unit$ on the empty alphabet $\Sigma_\Unit := \False$ with $encode~\unit := \nil$.
  \coqitem[Encode_bool] The type $\Bool$ is encoded on itself, i.e.\ $\Sigma_\Bool := \Bool$ and $encode(b):=[b]$.
  \coqitem[Encode_option] Let $X$ be encodable on $\Sigma_X$.  Then $\Option(X)$ is encodable on
    ...
    \end{alignat*}
  \coqitem[Encode_sum] Let $X$ be encodable on $\Sigma_X$ and $Y$ on $\Sigma_Y$.  Then $X+Y$ is encodable on
    \[ \Sigma_{X+Y} ::= \MS{INL} ~|~ \MS{INR} ~|~ (x:\Sigma_X) ~|~ (y:\Sigma_Y) \] With the retractions
    $RetrInl : \Sigma_X \hookrightarrow \Sigma_{X+Y}$, $RetInr : \Sigma_Y \hookrightarrow \Sigma_{X+Y}$ and
    \begin{alignat*}{3}
      &encode~&&(\inl x)&~:=~& \MS{INL} \cons encode(x) \\
      &encode~&&(\inr y)&~:=~& \MS{INR} \cons encode(y)
    \end{alignat*}
  \coqitem[Encode_pair] Let $X$ be encodable on $\Sigma_X$ and $Y$ on $\Sigma_Y$.  Then $X \times Y$ is encodable on
    ...
    \]
  \coqitem[Encode_list] Let $X$ be encodable on $\Sigma_X$.  Then $\List(X)$ is encodable on
    \[ \Sigma_{\List(X)} ::= \MS{NIL} ~|~ \MS{CONS} ~|~ (x:\Sigma_X) \]
    With the retraction $RetrList : \Sigma_X \hookrightarrow \Sigma_{\List(X)}$ and
    \begin{alignat*}{3}
      &encode~&&(\nil      ) &~:=~& [\MS{NIL}] \\
      &encode~&&(x \cons ls) &~:=~& \MS{CONS} \cons encode(x) \app encode(ls)
    \end{alignat*}
  \coqitem[Encode_nat] Natural numbers $\Nat$ are encodable on $\Sigma_\Nat ::= \MS{S} ~|~ \MS{0} $ with
    \begin{alignat*}{3}
      &encode~&&(0  ) &~:=~& [\MS{O}] \\
      &encode~&&(S~n) &~:=~& \MS{S} \cons encode(n)
    \end{alignat*}
  \end{enumerate}
  Note that we implicitly map the encoding functions in this definition.  Also note that $RetrLft : X \hookrightarrow X + Y$ is similar to
  $RetrInl : \Sigma_X \hookrightarrow \Sigma_{X+Y}$.
\end{definition}

This are all encodings we need in this thesis.  We say that $X$ is \textit{minimally encodable} on $\Sigma_X$, if $X$ is encodable on $\Sigma_X$
...
same alphabet, but there may also be several ways how to encode the same type on the same alphabet.  For example, the minimal alphabet of the type
$X+X$ is $\Sigma_{X+X}$.  Although there is only one way to encode $X+X$ on $\Sigma_{X+X}$, there are two possibilities how to encode $X$: using the
retraction $RetrInl$ or $RetrInr$.  We must deal with this problem, when we program and verify Turing machines, by explicitly specifying the right
retractions.
...
\begin{definition}[$\Sigma^+$][boundary]
  Let $\Sigma : \Type$.  Then $\Sigma^+ ::= \MS{START} ~|~ \MS{STOP} ~|~ \MS{UNKNOWN} ~|~ (s: \Sigma)$.
  \setCoqFilename{ProgrammingTuringMachines.TM.Code.ChangeAlphabet}%
  Also, we define the retraction $\coqlink[Retract_plus]{RetrPlus} : \Sigma \hookrightarrow \Sigma^+$.
  \setCoqFilename{ProgrammingTuringMachines.TM.Code.CodeTM}%
\end{definition}
...
$t \simeq x$, the head of $t$ stands on the start symbol.  To the right, there is the encoding, which is terminated by one stop-symbol.  To the left
of the head, there may be arbitrarily many ``rest'' symbols.
...
encoding.  This intuition is confirmed by the following lemma: \setCoqFilename{ProgrammingTuringMachines.TM.Code.ChangeAlphabet}%
\begin{lemma}[$surject$ and value-containment][contains_translate_tau]
  \label{lem:contains_translate_tau}
  Let $f: \Sigma \hookrightarrow \Tau$ be a retraction.  Let $x:X$, where $X$ is encodable on $\Sigma$, and $t:\Tape_{\Tau^+}$:
  \[
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.WriteValue}%

We define a wrapper around the machine $\MS{WriteString}$ from Section~\ref{sec:WriteString}.  We parametrise $\MS{WriteValue}$ over the encoding
$str : \List(\Sigma_X$) of a value $x:X$.\footnote{The reason why $\MS{WriteValue}$ is not parametrised over values $x:X$ is, that we would also need
  to parametrise the machine over the encoding function of $X$.  With this approach, only the correctness Lemma~\ref{lem:WriteValue_Sem} and the correctness
  relation are parametrised over the encoding.}
\begin{definition}[$\MS{WriteValue}$][WriteValue]
  \label{def:WriteValue}
  Let $str:\List(\Sigma)$
  \[
    \MS{WriteValue}(str) := \MS{WriteString}~\MS{L}~\left(\rev{\left(\MS{START} \cons \map{RetrPlus}{str} \app [\MS{STOP}]\right)}\right)
  \]
\end{definition}
The machine writes the stop symbol, the encoding $str$, and the start symbol in reversed order from right to left.  After the execution of the
machine, the tape contains the value $x$ with the encoding $str$, under the precondition that the tape was initially right.
\begin{lemma}[Correctness of $\MS{WriteValue}$][WriteValue_Sem]
  \label{lem:WriteValue_Sem}
  Let $X$ be encodable over $\Sigma_X$ and $str : \List(\Sigma)$.
  \[
...
\label{sec:match-nat}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.CaseNat}%
...
We define the $0$-constructor with $\MS{WriteValue}$:
...
\begin{definition}[$\MS{ConstrS}$][Constr_S]
  $\MS{ConstrS} := \MS{WriteMove}~\MS{S}~\MS{L} \Seq \MS{Write}~\MS{START}$
\end{definition}
...
$\MS{CaseNat}$ is the deconstructor machine for $\Nat$.  It reads a number from tape $t[0]$.  If it is $0$, $\MS{CaseNat}$ leaves the number unchanged
and terminates in the label $\false$.  Else, it decreases the number and terminates in~$\true$.

Initially, the tape $t[0]$ contains some number $n$.  Thus, the head of $t[0]$ must be on the start symbol.  The machine moves one step right and
reads the first symbol of the encoding.  This symbol either is $\MS{S}$ or $\MS{O}$.  If the machine read the symbol $\MS{O}$, this means that the
number $n$ is $0$, so the machine moves back to the start symbol and terminates in the label $\false$, indicating that the number was $0$.  On the
other side, if the machine read $\MS{S}$, this means the number $n$ is the successor of some number $n'$.  To decrement $n$, it overwrites the current
$\MS{S}$ with a new start symbol and terminates on this symbol in the label $\true$.

\begin{definition}[$\MS{CaseNat}$][CaseNat]
  \label{def:CaseNat}
  ~
...
$\MS{CaseNat} :=$
  $\MS{Move}~\MS{R}$; 
  $\Switch~\MS{Read}$ 
       ($\lambda(s:\Option(\Sigma_\Nat^+)).~\MS{match}~s$
          [$\Some {\MS{S}}$=>$\Return{(\MS{Write}~\MS{START})}{\true}$
          |$\Some {\MS{O}}$=>$\Return{(\MS{Move}~\MS{L})}{\false}$
          |_=>_ 
          ])
\end{lstlisting}
\end{definition}

Note, that the placeholder ``$\_$'' stands for some unspecified machine, e.g.\ in this case $\Return{\Nop}{b}$ where $b:\Bool$ can be chosen
arbitrarily.  We do not give this part explicitly here, because the premise of the correctness relation of $\MS{CaseNat}$ guaranties that there must
be a symbol under the head that is either $\MS{S}$ or $\MS{O}$, so this part of the machine must never execute.

\begin{lemma}[Correctness of $\MS{CaseNat}$][CaseNat_Sem]
...
convention has the advantage that when we apply the alphabet-lift with a retraction $f$, we would otherwise get assumptions in correctness proofs
like:
\begin{align*}
...
\end{align*}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.ChangeAlphabet}%
To show that $t'[i]$ contains the same value as $t[i]$, we would have to apply a \coqlink[contains_translate_eq]{lemma}.  Using this convention we get
this for free.%
\setCoqFilename{ProgrammingTuringMachines.TM.Code.CaseNat}%
...
Let $X$ be encodable on $\Sigma_X$ and $Y$ on $\Sigma_Y$.  $\MS{CaseSum} : \TM_{\Sigma_{X+Y}^+}^1(\Bool)$ reads a value $s:X+Y$.  If it is $\inl x$
(or $\inr~l$), it replaces the $\MS{INL}$ (or $\MS{INR}$) symbol with a new start symbol and terminates in $\true$ (or $\false$).

\setCoqFilename{ProgrammingTuringMachines.TM.Code.CaseSum}%
...
$\MS{CaseSum} :=$
  $\MS{Move}~R\Seq$
  $\Switch~\MS{Read}$
       ($\lambda(s:\Option(\Sigma_{X+Y}^+)).~\MS{match}~s$
...
\begin{lemma}[Correctness of $\MS{CaseSum}$][CaseSum_Sem]
...
We have one constructor machine for $\inl$ and $\inr$, respectively.  They are both analogous.  They overwrite the start symbol with $\MS{INL}$ or
$\MS{INR}$ and write a new start symbol one step further left.
\begin{definition}[$\MS{ConstrInl}$][Constr_inl]
  \label{def:Constr_inl}
  $\MS{ConstrInl} := \MS{WriteMove}~\MS{INL}~\MS{L} \Seq \MS{Write}~\MS{START}$.
\end{definition}
...
are also isomorphic.  We can use this fact to derive a deconstructor machine and constructor machines for the type $\Option(X)$.  Let
...
\begin{definition}[$\MS{CaseOption}$][CaseOption]
  ...
  ~
  \[
    \MS{CaseOption} := \If{(\LiftAlphabet{\MS{CaseSum}}{f})}{(\Return{\Nop}{\true})}({\Return{(\MS{Move}~\MS{R})}{\false})}
  \]
\end{definition}

\begin{lemma}[Correctness of $\MS{CaseOption}$][CaseOption_Sem]
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.CasePair}%
\begin{definition}[$\MS{CasePair}$][CasePair]
  \label{def:CasePair}
  ~
  \begin{alignat*}{2}
    &\MS{CasePair}&~:=~& \LiftTapes{(\MS{WriteMove}~\MS{STOP}~\MS{L})}{\Vector{1}} \Seq \\
    &              &    & \LiftTapes{(\MS{MoveToSymbol}~f~id \Seq \MS{Move}~\MS{L})}{\Vector{0}} \Seq \\
    &              &    & \MS{CopySymbolsL}~g\Seq \\
    &              &    & \LiftTapes{(\MS{MoveToSymbol}~f~id \Seq \MS{Move}~\MS{L} \Seq \MS{Write}~\MS{START})}{\Vector{0}}
  \end{alignat*}
...
\begin{lemma}[Correctness of $\MS{CasePair}$][CasePair_Realise]
  ...
  \]
\end{lemma}
\begin{lemma}[Running time of $\MS{CasePair}$][CasePair_Terminates]
...
\begin{definition}[$\MS{ConstrPair}$][Constr_pair]
  ...
  ~
  \[
    \MS{ConstrPair} := \LiftTapes{(\MS{MoveToSymbol}~h~id)}{\Vector{0}} \Seq \MS{CopySymbolsL}~g
  \]
...
\begin{lemma}[Correctness of $\MS{ConstrPair}$][Constr_pair_Realise]
  ...
  \]
\end{lemma}
\begin{lemma}[Running time of $\MS{ConstrPair}$][Constr_pair_Terminates]
...
\label{sec:CaseList}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.CaseList}%
...
tapes unchanged.  If the list is $x \cons xs$, it moves $x$ to tape $1$ and terminates in the label $\true$.

\begin{lemma}[Correctness of $\MS{CaseList}$][CaseList_Realise]
...
The ``$\cons$'' constructor machine $\MS{ConstrCons}$ expects a list on tape $0$ and a value $x:X$ on tape $1$.  It moves the head of tape $1$ to the
last symbol of the encoding of $x$ and copies them on tape $0$ from right to left.  Thereby, it overwrites the current start symbol on tape $1$, but
also copies the start symbol from tape $1$ to tape $0$.  After that, the machine overwrites this start symbol with $\MS{CONS}$ and writes a new start
symbol on tape $0$.

\begin{definition}[$\MS{ConstrCons}$][Constr_cons]
  \label{def:Constr_cons}
  \begin{alignat*}{2}
    &\MS{ConstrCons}&~:=~& \LiftTapes{(\MS{MoveToSymbol}~h~id \Seq \MS{Move}~\MS{L})}{\Vector{1}} \Seq \\
    &               &    & \LiftTapes{(\MS{CopySymbolsL~h'})}{\Vector{1;0}} \Seq \\
    &               &    & \LiftTapes{(\MS{WriteMove}~\MS{CONS}~\MS{L} \Seq \MS{Write}~\MS{START})}{\Vector{0}}
  \end{alignat*}
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.CaseFin}%
...
\begin{lstlisting}[style=semicoqstyle]
$\MS{CaseFin} :=$
  $\MS{Move}~\MS{R} \Seq \Switch~\MS{Read}$($\lambda s.~\MS{match}~s$[$\Some{x} \Rightarrow \Return{(\MS{Move}~\MS{R})}{x}$|_$\Rightarrow\Return{\Nop}{def}$])
\end{lstlisting}
\end{definition}

\begin{lemma}[Correctness of $\MS{CaseFin}$][CaseFin_Sem]
...
\setCoqFilename{ProgrammingTuringMachines.TM.Code.Copy}%
...
\begin{lemma}[Running time of $\MS{CopyValue}$][CopyValue_Terminates]
...
In Section~\ref{sec:value-containment}, we observed that the containment relation is ambiguous: if $X$ is minimally encodable on $\Sigma_X$, there may
be two retractions $f_1, f_2 : \Sigma_X \hookrightarrow \Tau$ on the alphabet $\Tau$.  In Section~\ref{sec:MoveToSymbol}, we gave the
$\MS{MoveToSymbol}$ machine the feature to translate the symbols it reads.  Using this feature, we define a machine that translates between the two
...
\begin{fact}
  If the retractions $f_1 : X \hookrightarrow Z$ and $f_2 : Y \hookrightarrow Z$ have disjoint images in $Z$, then we can define a retraction
  $(f_1+f_2) : X+Y \hookrightarrow Z$.
\end{fact}
...
As noted above, when we speak of ``programming'' Turing machines, we mean that we use tapes as registers, and use the control-flow operators and
lifting operators to compose machines.  Moreover, we only use machines that directly change the values of registers (tapes).  We have machines that do
a case-distinction on values, and machines that apply constructors to the value that a tape contains.
...
continue or break out of the loop.  Tail-recursive functions can easily be transformed into ``do-while'' loops.  Thus, when we translate functions to
Turing machines, we first have to implement the function as a tail-recursive function, which is then translated to a Turing machine in ``programming
style''.
...
kind of tape has an invariant, which is encoded in the correctness relation of the machine.  For input tapes, we have the invariant that the value on
the tape is not changed by the computation.  Output tapes are initially right, and contain a value after the execution.  Internal tapes are right
before and after the execution.

The deconstructor machines do case-distinctions over the value on a tape, and may alter the value on the tape thereby.  Each deconstruction is
...
With this distinction of input, output, and internal tapes, we can formalise a convention for functional computation of binary functions: The tapes
...
\begin{definition}[Functional computation running time relation][Computes2_T]
...
copies of $x$ and $y$ and additional internal tapes, are resetted by $M$.
...
\subsection{Addition}
\label{sec:Add}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.NatTM}%
...
Because the machine only operates on natural numbers, we choose $\Sigma_\Nat^+$ as the alphabet of $\MS{Add}$ and all its sub-machines.
...
\begin{definition}[$\MS{AddStep}$][Add_Step]
  ...
  \[
    \MS{AddStep} :=
    \If{(\LiftTapes{\MS{CaseNat}}{\Vector{1}})}{(\Return{(\LiftTapes{\MS{ConstrS}}{\Vector{0}})}{\None})}{(\Return{\Nop}{\Some\unit})}
  \]
\end{definition}
...
The running time of $\MS{AddLoop}$ must be shown separately.  We know that the loop is executed $b+1$ times, and each iteration takes $9$ steps.  We
have to add $1$ step for each re-iteration of the loop.  Thus, the total step number is $9+10 \cdot b$.
\begin{lemma}[Running time of $\MS{AddLoop}$][Add_Loop_Terminates]
...
For the running time function of $\MS{Add}$, we have to add linear components for copying $m$ and $n$, a constant for $\MS{Reset}$, and $1$ step for
each sequential composition operator.
\begin{lemma}[Running time of $\MS{Add}$][Add_Terminates]
...
\begin{table}[h]
  \centering
  \begin{tabular}{l|l|l}
    Sub-Machine & Running time & Accumulated running time \\ \hline
...
The tape-assignment is visualised in Table~\ref{tab:tapes-Mult}.  The step machine, $\MS{MultStep}$, has only access to the copy of $m$, which is
...
\end{lstlisting}
\end{lemma}
Note that in the correctness Lemma~\ref{lem:Add_Step_Sem} of $\MS{AddStep}$, we also prove termination in constant running time.  This is not true for
$\MS{MultStep}$, because it calls $\MS{Add}$, which has not constant running time.
...
\begin{lemma}[Running time of $\MS{Mult}$][Mult_Terminates]
...
\label{sec:SumTM}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.SumTM}%
...
the retractions $f_M \circ f_X$ and $f_{X+Y} \circ RetrInl$, see Figure~\ref{fig:retracts-MapSum}.  These retractions might be extensionally equal for
concrete choices of alphabets, but we want to be as general as possible.  As a consequence, we need to translate between these representations, using
$\MS{Translate}$ of Section~\ref{sec:Translate}.  Note that there is only one way to encode $Z$ on $\Sigma$, namely via $f_M \circ f_Z $.
...
\begin{lemma}[Correctness of $\MS{MapSum}$][MapSum_Computes]
...
\begin{figure}
  \center
  \begin{tikzcd}
    \Sigma_X \arrow[r, "f_X", hook] \arrow[d, "RetrInl", hook, swap] & \Sigma_M \arrow[r, "f_M", hook] & \Sigma \\
    \Sigma_{X+Y} \arrow[urr, "f_{X+Y}", hook, swap]
  \end{tikzcd}
  \caption{Non-commutative diagram of the retractions involved in the definition of $\MS{MapSum}$}
  \label{fig:retracts-MapSum}
  \begin{tikzcd}
    & \Sigma_{\List(X)} \arrow[dr, "f_{\List(X)}", hook] & \\
    \Sigma_X \arrow[ur, "RetrList", hook] \arrow[dr, "RetrOpt", hook, swap] & & \Sigma \\
    & \Sigma_{\Option(X)} \arrow[ur, "f_{\Option(X)}", hook] & \Sigma_\Nat \arrow[u, "f_\Nat", hook, swap]
  \end{tikzcd}
  \caption{Retractions involved in the definition of $\MS{Nth}$}
  \label{fig:retracts-Nth}
\end{figure}


\subsection{List Access}
\label{sec:Nth}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.ListTM}%

In this section, we show how to implement machines that compute the function $nth : \List(X) \to \Nat \to \Option(X)$.
...
$X$ on $\Sigma$: via $f_{\Option(X)} \circ RetrOpt$ and $f_{\List(X)} \circ RetrList$, see Figure~\ref{fig:retracts-Nth}.
...
constructor to the head.  Note that before we can apply the constructor,~$x$ has to be translated to the $\Sigma_{\Option(X)}$ alphabet.  In case~$n$
was non-zero, if the list is empty, the machine applies the~$\None$ constructor to the output tape.  Else, it deletes the head (which was temporary
stored on the output tape) and continues the loop.
...
Now, we can prove the correctness of the loop, $\MS{NthLoop} := \While~\MS{NthStep}$.  When the loop terminates, the value of the variable $n$ is
$n - (S~\length{xs})$ and the new value of $xs$ is $skipn~(S~n)~xs$, where $skipn : \Nat \to \List(X) \to \List(X)$ applies $\tl$ $n$-times.  Note
...
with the copies of $n$ and $xs$.  Note that for the correctness of $\MS{Reset}$ it is only important that there is a value on the tape.  Only the
running time (which we omit here) does depend on the actual value.
\begin{definition}[$\MS{Nth}$][Nth]
  ...
  \]
\end{definition}
Note that we copy and reset values of different types.  We have to insert this semantic information in correctness proofs when we apply the respective
correctness lemmas of $\MS{Copy}$ and $\MS{Reset}$ in the relation-derivation phase of correctness proofs.
...
% For brevity, we omit the concrete running time function.
% \begin{lemma}[Running time of $\MS{Nth}$][Nth_Terminates]
