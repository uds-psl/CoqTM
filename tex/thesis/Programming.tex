...
registers can usually only contain numbers.  %We will define what it means that a tape contains a value.
...
For each type $X$, we have an alphabet $\Sigma_X$.  If $X$ is encodable on $\Sigma_X$, then $P(X)$ is defined on an alphabet $\Sigma_{P(X)}$.  We
define encodings for several types:
\begin{definition}[Basic encodings]
  \label{def:basic-encodings}
  ~
  \begin{enumerate}
  \coqitem[Encode_unit] We encode $\Unit$ on the empty alphabet $\False$ with $encode~\unit := \nil$
  \coqitem[Encode_fin] The types $\Bool$ and $\Fin_k$ are encoded on themself, i.e.\ $encode(x):=[x]$.
...
\label{sec:match-nat}
\setCoqFilename{ProgrammingTuringMachines.TM.Code.Nat}%
...
is not a recursive data type and the information that $o=\None$ is already encoded in the partition in which $\MS{CaseOption}$ terminates.  So in
...
\begin{lemma}[Correctness of $\MS{CaseFin}$][CaseFin_Sem]
  ...
  \[
    CaseFinRel := \lambda t~(y,t').~\forall(x:\Sigma).~t[0] \simeq x \rightarrow isRight~t'[0] ~\land~ y=x.
  \]
\end{lemma}
