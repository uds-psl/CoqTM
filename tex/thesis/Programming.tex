...
\begin{definition}[Basic encodings]
  ...
  ~
  \begin{enumerate}
  \item We encode $\Unit$ over the empty alphabet $\False$: $encode~\unit := \nil$
...
This are all encodings we need.  Generally, we say that $X$ is minimally encodable over $\Sigma_X$, if $X$ is encodable over $\Sigma_X$ according to
Definition~\ref{def:basic-encodings}.  The mapping encodings introduces ambiguity: It is not only possible to encode multiple types on the same
...
\begin{definition}[$t \simeq x$]
  ...
  \]
  In case the encoding is not clear, we write $t \simeq_f x$ with $t:\Tape_{\Tau^+}$ and $x:X$, when $X$ is minimal encodable on $\Sigma$ and
  $f: \Sigma \hookrightarrow \Tau$.
\end{definition}
...
\begin{lemma}[Correctness of $\MS{MatchFin}$]
