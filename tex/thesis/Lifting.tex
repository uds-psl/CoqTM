...
\todo{Maybe use another example for motivating the alphabet-lift, since this example is not implemented}
...
\begin{definition}[Vector filling]
...
\begin{definition}[$\LiftTapes{M}{I}$]
  \label{def:LiftTapes}
  Let $M:\TM_\Sigma^m(F)$.
  \begin{alignat*}{2}
...
\begin{lemma}[Correctness of $\LiftTapes{M}{I}$]
...
\begin{lemma}[Runtime of $\LiftTapes{M}{I}$]
...
\begin{definition}[$\LiftAlphabet{M}{(f,def)}$]
...
\begin{lemma}[Correctness of $\LiftAlphabet{M}{(f,def)}$]
...
\begin{lemma}[Runtime of $\LiftAlphabet{M}{(f,def)}$]
