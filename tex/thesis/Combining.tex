...
operator on partitioned machines, called $\MS{Switch}$ and derive sequential composition and conditional.

\begin{definition}[$\MS{Switch}~M~f$][Switch]
  \label{def:Switch}
  Let $M : \TM_\Sigma^n(F)$ and $f : F \to \TM_\Sigma^n(F')$.  We define the machine $\MS{Switch}~M~f : \TM_\Sigma^n(F')$ with the following
  components:
  \begin{alignat*}{3}
...
In Definition~\ref{def:Switch}, the $part$ value for $\inl$ is unimportant, because the lifted states of $M$ are not terminating states for
...
to the injection of the start state of the machine $f(part_M~q)$.  When $\MS{Switch}~M~f$ reaches a state that is the injection of a final state of a
...
\begin{lemma}[Correctness of $\MS{Switch}~M~f$][Switch_Realise]
  ...
  with
  \[
    SwitchRel~R~R' := \bigcup_{y:F} \bigl( R\at y \circ R'~y \bigr)
  \]
\end{lemma}
...
partitioned relation $R'~y \subseteq \Tape_\Sigma^n \times F' \times \Tape_\Sigma^n$.  This means that $\Switch~M~f$ terminates in a partition of
$f~y$, which has type $F'$.

To specify the running time of $\MS{Switch}~M~f$, we need to know the running time relation in that $M$ terminates, and for each $y:F$ the running time
...
implementing the more general $\Switch$ combinator, we solve both problems.  We no longer have to specify concrete machine states in the definition and
verification of machines.  Moreover, we often exploit the convenient generality of the $\Switch$ operator.  For example, it can be used to implement a
...
the lifted steps of the execution of $f~y$.  We give the concrete lifting functions from $M$ to $\Switch$ and from $M~y$ to $\Switch$:
\setCoqFilename{ProgrammingTuringMachines.TM.Combinators.Switch}%
\begin{definition}[Liftings of $\Switch$][lift_confL]
  We define the functions \\$liftL : \Conf_M \to \Conf_\Switch$ and $liftR_y : \Conf_{f(y)} \to \Conf_\Switch$ for all $y:F$:
  \begin{alignat*}{2}
    & liftL   & (q, t) &:= (\inl q,      t) \\
...
For the sequential part, we also have to define the lifted halting function of \\$haltConfL : \Conf_\Switch \to \Bool$:
...
\begin{proof}
  ...
  \item $\Loop~step_\Switch~haltConf_\Switch~(liftL~c1)~(1+k_2) =
    ...
    $$\Loop~step_\Switch~haltConf_\Switch~liftR \bigl(\MS{initConf}_{f(part_M~q_1)}~t_1 \bigr)~k_2 = \Some{liftR(c_2)}$$
