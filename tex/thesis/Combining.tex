...
We define control flow operators as \textit{first-class citizen} in our meta-logic.\footnote{The calculus of (co-)inductive constructions (CIC) is the
  theoretical foundation of Coq.} As a result, we get we get a \textit{shallow-embedded} language for programming multi-tape Turing machines in an
imperative way.  The primitive machines defined in Chapter~\ref{chap:basic} serve as the ``primitive instructions'' of our language.  Another possible
