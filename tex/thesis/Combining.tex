...
\begin{lemma}[Correctness of $\MS{Match}~M~M'$]
  ...
  \[
    \MS{Match}~M~M' \vDash MatchRel~R~R'
  \]
  with
  \[
...
\begin{lemma}[Correctness of $\MS{Match}~M~M'$ in constant time]
  ...
  \[ \MS{Match}~M~M' \vDash^{1+k_1+k_2} MatchRel~R~R'. \]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Correctness of $\MS{While}$]
  ...
  If $M \vDash R$, then $\While~M \vDash WhileRel~M$, where
