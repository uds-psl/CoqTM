...
In Chapter \ref{chap:definitions}, we have defined the notion of multi-tape Turing machines and basic machines.  In this chapter, we define control
flow operators, like ``match'', ``if then else'', ``sequential composition'', and ``while''.  As a result, we get a shallow-embeded language for
programming multi-tape Turing machines in an imperative way.
...
\todo{Maybe use parenthesises, e.g.\ $\Match(M,M')$ and $MatchRel(R,R')$ for better readability}
...
\begin{lemma}[Correctness of $\MS{Match}~M~M'$]
  ...
  If $M \Realise R$ and $M'~y \Realise R'~y$ for all $y:F$, then
  \[
    \MS{Match}~M~M' \Realise MatchRel~R~R'
  \]
  with
  \[
...
partitioned relation $R'~y \subseteq \Tape_\Sigma^n \times F' \times \Tape_\Sigma^n$.  This means that $\Match~M~M'$ terminates in a partition of
...
\begin{lemma}[Runtime of $\MS{Match}~M~M'$]
  ...
  $T'~y \subseteq \Tape_\Sigma^n \times \Nat$ for all $y:F$.  If $M \Realise R$, $M \TerminatesIn T$, and $M'~y \TerminatesIn T'~y$ for all $y:F$, then
  $\MS{Match}~M~M' \TerminatesIn \MS{MatchT}~R~T~T'$, where
  \[
    \MS{MatchT}~R~T~T' :=
...
\begin{lemma}[Correctness of $\MS{Match}~M~M'$ in constant time]
  \label{lem:Match_RealiseIn}
  Let $k_1, k_2:\Nat$.\\
  If $M \RealiseIn{k_1} R$ and $M'~y \RealiseIn{k_2} R'~y$ for every $y:F$, then
  \[ \MS{Match}~M~M' \RealiseIn{1+k_1+k_2} MatchRel~R~R'. \]
\end{lemma}
\begin{proof}
...
\begin{lemma}[Correctness of conditional]
...
\begin{lemma}[Correctness of sequential composition]
...
\begin{lemma}[Merging parts of executions of $\Match$]
  ...
  Let $t : \Tape_\Sigma^n$, $k_1, k_2:\Nat$, $c_1 : \Conf_M$ and $c_2 : \Conf_{M'(y)}$.
  \begin{alignat*}{1}
...
\begin{proof}
  ...
  \item $\Loop~step_\Match~haltConf_\Match~(liftL~c1)~(1+k_2) =
    ...
    $liftL~c_1$ to\\
    $liftR~\bigl(\MS{initConf}_{M'(part_M~q_1)}~t' \bigr)$.  It remains to show that
    $$\Loop~step_\Match~haltConf_\Match~liftR \bigl(\MS{initConf}_{M'(part_M~q_1)}~t' \bigr)~k_2 = \Some{liftR(c_2)}.$$
...
However, if we know that $M$ realises $R$ and terminates in $T$, we can show $\While~M \TerminatesIn T'$ for another runtime relation $T'$, if $T'$
``decreases'' in terms of $R$ and $T$:

\begin{lemma}[Runtime of $\While~M$]
  \label{lem:While_TerminatesIn}
  If $M \Realise R$, $M \TerminatesIn T$.  We can show $\While~M \TerminatesIn T'$ under the following assumption:
  \begin{multline*}
    \forall~t~i.~
    T'~t~i \rightarrow
    \exists~i_1.~
...
Note that we define concrete correctness and termination relations for all combinators, but there is no known ``direct'' runtime Relation for
...
However, when we try to prove $\While~M \TerminatesIn WhileT~R~T$, we come to a goal where we have $R~t~(\Some y, t')$ and $R~t~(part_M(q''), t'')$, for
another final configuration $(q'',t'')$.  Thus, we can only prove $\While~M \TerminatesIn WhileT~R~T$, if we assume that $R$ is functional:

\begin{lemma}[Direct runtime relation for $\While$]
  \label{lem:While_TerminatesIn'}
  If $M \Realise R$, $M \TerminatesIn T$, and $R$ is functional on tapes in $T$, i.e.:
  \[
    \forall t~k.~T~t~k \rightarrow \forall t_1~t_2~y_1~y_2.~R~t~(y_1, t_1) \rightarrow R~t~(y_2, t_2) \rightarrow y_1 = y_2 \land t_1 = t_2.
  \]
  Then $\While~M \TerminatesIn WhileT~R~T$.
\end{lemma}

However, our correctness relations are not always functional on the tapes in $T$.  We encode preconditions in the relations.  If the precondition is
not satisfied for $t$, then the proposition $R~t~(y,t')$ holds for every $t'$ and $y$.  Also, Lemma~\ref{lem:While_TerminatesIn} follows from
Lemma~\ref{lem:While_TerminatesIn'}, however, only under the functionality assumption.  Therefore, we always use Lemma~\ref{lem:While_TerminatesIn}.
...
between $\Match$ and $\While$ is that $\While$ can execute $M$ arbitrary often.  As a consequence, we also need complete induction on step-numbers,
...
\begin{lemma}[Splitting, continue case]
  ...
  \begin{alignat*}{1}
    ...
    & \exists k'.~k = 1+k' ~\land~ \While(c_1) \terminates^{k'} c_2.
  \end{alignat*}
\end{lemma}
\begin{proof}
...
The operators of the above sections of this Chapter modified the behaviour of the machine.  However, we can also define simple operators that modify
