...
Note that in the relation $\MS{MatchRel}$, we compose the unpartitioned relation $R \at y \subseteq \Tape_\Sigma^n \times \Tape_\Sigma^n$ with the partitioned
...
\end{lemma}
\begin{proof}
  By induction over $k1:\Nat$, using Lemma~\ref{lem:loop}~(\ref{lem:loop_monotone}).
\end{proof}
...
We simply write $\While$ instead of $\While$ in this section.  Since we have $\Conf_M = \Conf_\While$, we also only write $\Conf$.
