...
\begin{lemma}[Runtime of $\MS{Match}~M~f$][Match_TerminatesIn]
...
machine $M_2$.
\setCoqFilename{FormalComplexity.TM.Combinators.SequentialComposition}%
...
\begin{lemma}[Runtime of sequential composition][Seq_TerminatesIn]
...
For the conditional $\If{M_1}{M_2}{M_3}$ with $M_1 : \TM_\Sigma^n(\Bool)$, $M_2, M_3 : \TM_\Sigma^n(F)$, the function
...
\begin{lemma}[Runtime of the conditional][If_TerminatesIn]
...
\begin{fact}[Alternative specification of $\While~M$][OtherWhileRel]
  ~
  \[
...
The runtime lemma of $\While$ is dual.  We define a co-inductive relation $WhileT~R~T'$, where $R$ is the relation that $M$ realises and $T'$ is the
runtime relation in that $M$ terminates.
\begin{lemma}[Runtime of $\While~M$][While_TerminatesIn]
...
\setCoqFilename{FormalComplexity.TM.Combinators.Mirror}%
...
The operators of the above sections of this Chapter modified the behaviour of the machine.  We can also define simple operators that modify the
