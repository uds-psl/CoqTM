...
We define control flow operators as \textit{first-class citizen} in our meta-logic.\footnote{The calculus of (co-)inductive constructions (CIC), which
  is the theoretical foundation of Coq} As a result, we get we get a \textit{shallow-embedded} language for programming multi-tape Turing machines in
an imperative way.  The basic machines defined in Chapter~\ref{chap:basic} serve as the ``primitive instructions'' of our language.  Another possible
approach, called \textit{deep embedding}, is to define a fixed syntax of a language as an inductive data type.  We then would need a compiler from
syntax trees of Turing machine programs to actual Turing machines.  This compiler would have to be extended, whenever we add a new feature to our
language.  In our approach, we can just ``extend'' our language by defining a function in the meta-logic, and we use the same verification technique
(i.e.\ realisation and termination) on each stage of the development.

% Advantages and disadvantages of shallow vs deep embedding
% Shallow:
% + Extensible
% + All features of the meta logic (i.e. Coq)
% + Thus, it is extendable, e.g. we can easily add new combinators without changing any other definitions
% + We use the same verification techniques on every "stage" of the development
% Deep:
% + We can define meta-properties about programs, e.g. the size of the abstract syntax tree (needless)
% - We had to implement a compiler from the abstract syntax tree to concrete Turing machines
% - We had to prove that the combinators are correct with respect to the semantics of the compiled operators
% - We have to amend the compiler, whenever we want to add a new feature to the language

\section{$\MS{Match}$}
\label{sec:match}

\setCoqFilename{FormalComplexity.TM.Combinators.Match}
...
operator on partitioned machines, called $\MS{Match}$ and derive sequential composition and conditional.
...
\begin{definition}[$\MS{Match}~M~f$][Match]
  ...
  \begin{alignat*}{3}
    ...
    \begin{cases}
      ...
      \Let{(q', acts) := \delta_M(q, s)}{\left(\inl q', acts \right)} & \lnot halt_M(q)
    \end{cases} \\
    \delta ~&(\inr q, s) &~:=~& \Let{(q', acts) := \delta_{f(\pi_1 q)} (\pi_2 q, s)}{\bigl( \inr (\pi_1 q, q'), acts \bigr)} \\
    halt   ~&(\inl  q)   &~:=~& \false \\
...
In Definition~\ref{def:Match}, the $part$ value for $\inl$ is unimportant, because the lifted states of $M$ are not terminating states for
$\MS{Match}~M~f$.  We just use a canonical value.
...
\begin{lemma}[Correctness of $\MS{Match}~M~f$][Match_Realise]
...
To specify the running time of $\MS{Match}~M~f$, we need to know the running time relation in that $M$ terminates, and for each $y:F$ the running time
relation of $f~y$.  We also need to know the correctness relation of $M$, because the running time of $f~y$ depends on the output of $M$, which is the
input of $f~y$, and the choice of the case machine depends on the partition of the terminating state of $M$.

\begin{lemma}[Running Time of $\MS{Match}~M~f$][Match_TerminatesIn]
...
\begin{lemma}[Correctness of $\MS{Match}~M~f$ in constant time][Match_RealiseIn]
...
$M_1 \Seq M_2$ with $M_1 : \TM_\Sigma^n(F)$ and $M_2 : \TM_\Sigma^n(F')$, the function $f : F \to \TM_\Sigma^n(F')$, maps all partitions to the
machine $M_2$.  \setCoqFilename{FormalComplexity.TM.Combinators.SequentialComposition}%
...
Lemma~\ref{lem:Match_Terminates}.
...
For the conditional $\If{M_1}{M_2}{M_3}$ with $M_1 : \TM_\Sigma^n(\Bool)$, $M_2, M_3 : \TM_\Sigma^n(F)$, the function $f : \Bool \to \TM_\Sigma^n(F)$
...
\setCoqFilename{FormalComplexity.TM.Combinators.If}%
...
differences.  First, they define and verify each operator separately.  Secondly, they do not consider state partitioning functions.  Their conditional
operator takes one concrete ``negative'' state, i.e.\ the machine $M_3$ is only executed if $M_1$ terminated in this state.  This makes programming
and reasoning about concrete machines tedious, because complex machines also have many states.  By introducing state-partitioned machines and by
implementing the more general $\Match$ combinator, we solve both problems.  We no longer have to specify concrete machine states in the definition and
verification of machines.  Moreover, we often exploit the convenient generality of the $\Match$ operator.  For example, it can be used to implement a
machine that copies a symbol from one tape to another tape, as demonstrated in Chapter~\ref{chap:combining}.  Also note, that Asperti and
Ricciotti~\cite{asperti2015} have no notion of time complexity; their strong notion of realisation only implies termination in an uncertain number of
steps.
...
The execution steps of $\Match$ are essentially a sequence of first, the lifted steps of execution of $M$, second, a ``nop'' transition, and third,
the lifted steps of the execution of $f~y$.  We give the concrete lifting functions from $M$ to $\Match$ and from $M~y$ to $\Match$:
\setCoqFilename{FormalComplexity.TM.Combinators.Match}%
\begin{definition}[Liftings of $\Match$][lift_confL]
...
\begin{lemma}[Merging parts of executions of $\Match$][Match_merge]
...
\begin{lemma}[Splitting execution of $\Match$][Match_split]
...
\section{$\MS{While}$}
\label{sec:While}
\setCoqFilename{FormalComplexity.TM.Combinators.While}%
...
$L$ and $R$.  \setCoqFilename{FormalComplexity.TM.Combinators.Mirror}%
...
\begin{lemma}[Correctness of $\MS{Mirror}$]
  ...
  \[
    MirrorRel~R := \lambda t~(y, t').~ R~(\map{\MS{mirror}}{t})~(y, \map{\MS{mirror}}{t'})
  \]
\end{lemma}
\begin{proof}
...
\setCoqFilename{FormalComplexity.TM.Combinators.Combinators}%
...
\begin{definition}[$\MS{ChangePartition}$][ChangePartition]
  ...
  \[ \MS{ChangePartition}~M~g := \left(M,~ \lambda (q:Q_M).~g(part_M~q)\right) \]
\end{definition}
...
and $\MS{Return}$.
\begin{lemma}[Correctness of $\MS{ChangePartition}$ and $\MS{Return}$][ChangePartition_Realise]
  If $M \Realise R$, then
  \begin{alignat*}{1}
