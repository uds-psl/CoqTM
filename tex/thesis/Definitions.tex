...
$A = (a, b, c, \cdots)$, we use subscripts, i.e.\ $a_A$, for the projections.  We use the symbols $\None$ and $\Some x$ as elements of the type
...
\begin{lemma}[Basic facts about $\Loop$]
  ...
  \item \label{lem:loop_injective}
    \coqlink[loop_injective]{If $\Loop~f~h~a~k = \Some{b}$ and $\Loop~f~h~a~l = \Some{b'}$, then $b = b'$.}
...
\begin{definition}[Termination on a runtime relation][TerminatesIn]
