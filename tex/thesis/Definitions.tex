...
\begin{lemma}[Correctness of $\MS{Null}$]
  \label{lem:Null_Sem} $\MS{Null} \RealiseIn0 \MS{NullRel}$ with $NullRel := \lambda~t~t'.~ \True$.
\end{lemma}
\begin{proof}
...
\begin{lemma}[Correctness of $\MS{DoAct}$]
  \label{lem:DoAct_Sem} $\MS{DoAct}~a \RealiseIn1 \MS{DoActRel}~a$ with
  \[
...
\begin{lemma}[Correctness of $\MS{Read}$]
  \label{lem:Read_Sem} $\MS{Read}~f \RealiseIn0 \MS{ReadRel}~f$ with
  \[
