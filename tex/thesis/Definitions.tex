...
\begin{lemma}[Specification of realisation in constant time][Realise_total]
...
\todo{This could get its own chapter, because basic machines need another motivation.  Duplicating the motivation text on top of the chapter here is
  not wise.  Also, basic machines are unrelated to the definition.}

\subsection{$\MS{Null}$}
\label{sec:Null}

\setCoqFilename{FormalComplexity.TM.Basic.Null}

A very simple class of machines is the zero-tape $\MS{Null}$ machine.  The machine $\MS{Null}_\Sigma$ is parametrised over the alphabet.  Because
...
\setCoqFilename{FormalComplexity.TM.Basic.Mono}
