...
\begin{definition}[$\MS{LookupStep}$][Lookup_Step]
  ...
  \begin{align*}
    ...
    & \quad \MS{Then}~\MS{If}~(\LiftBoth{\MS{MatchOption}}{f_{heap} \circ RetrList}{\Vector{4}}) \\
    & \quad \phantom{\MS{Then}}~\MS{Then}~\LiftBoth{\MS{MatchPair}}{f_{heap} \circ RetrList \circ RetrOpt}{\Vector{4;3}} \Seq \\
...
\begin{definition}[$\MS{SplitBody}$][JumpTarget_Step]
  ...
  \begin{alignat*}{1}
    ...
    & \quad \MS{Then}~\Switch~(\LiftBoth{\MS{MatchCom}}{RetrList}{\Vector{3}}) \\
