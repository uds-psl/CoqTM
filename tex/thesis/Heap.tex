...
runtime.  We show that this machine reduces the halting problem of the abstract machine to the halting problem of multi-tape Turing machines.
...
\label{sec:heap-def}
\setCoqFilename{FormalComplexity.TM.LM.Semantics}%
...
\setCoqFilename{FormalComplexity.TM.LM.Alphabets}%
...
First, we derive constructor and deconstructor machines for tokens.  As the type of tokens essentially is the sum of two encodable types ($ATok$ and
$\Nat$), we can combine constructors and deconstructors from Section~\ref{sec:constructors-deconstructors}.
\setCoqFilename{FormalComplexity.TM.LM.MatchTok}%
\begin{definition}[$\MS{MatchTok}$][MatchTok]
  ...
  \end{alignat*}
\end{definition}
\begin{lemma}[Correctness of $\MS{MatchTok}$][MatchTok_Sem]
...
\label{sec:Lookup}
\setCoqFilename{FormalComplexity.TM.LM.LookupTM}%
...
\begin{enumerate}
\item an address of the pointer of an heap on the stack alphabet:
  \[
...
$g = (g,b)$ and makes a case-distinction over $n$.  If $n=0$, $\MS{LookupStep}$ resets the tapes for $b$ and $n$, translates $g$ from $f_{Clos'}$ to
...
%%% BUG! When I move these two tables on the same page, aligns around the page get HUGE extra vertical spaces.
%%% This is also buggy, if all table floats are moved on a dedicated floating-page.
%%% THIS SUCKS!!!
\begin{table}[t!]
  \centering
  \scriptsize
...
\label{sec:SplitBody}
\setCoqFilename{FormalComplexity.TM.LM.JumpTargetTM}%
...
\begin{definition}[$\MS{SplitBody}$][JumpTarget_Step]
  ...
  %   & \quad \phantom{\MS{Then}~(\lambda~[~ \Some{\MS{RET} \Rightarrow}}~\MS{Then}~ \Return{(\LiftTapes{\MS{AppAtok~\MS{RET}}}{\Vector{1;4}})}{\None} \\
  %   & \quad \phantom{\MS{Then}~(\lambda~[~ \Some{\MS{RET} \Rightarrow}}~\MS{Else}~~~ \Return{(\LiftTapes{\MS{Reset}}{\Vector{2}})}{\Some\true} \\
  %   & \quad \phantom{\MS{Then}~(\lambda}~|~ \Some{\MS{LAM}} \Rightarrow \Return{(\LiftBoth{\MS{ConstrS}}{RetrRgt \circ RetrList}{\Vector{2}} \Seq \MS{AppATok}~\MS{LAM})}{\None} \\
  %   & \quad \phantom{\MS{Then}~(\lambda}~|~ \Some{\MS{APP}} \Rightarrow \Return{(\MS{AppATok}~\MS{APP})}{\None} \\
  %   & \quad \phantom{\MS{Then}~(\lambda}~|~ \None \Rightarrow \Return{(\LiftBoth{\MS{ConstrVar}}{RetrRgt \circ RetrList}{\Vector{3}} \Seq \LiftTapes{\MS{AppTok}}{1;3;4})}{\None} \\
  %   & \quad \phantom{\MS{Then}~(\lambda}~]) \\
  %   & \quad \MS{Else}~~~ \Return{\Nop}{\Some\false}.
  % \end{alignat*}
...
\subsection{$\MS{Step}$}
\label{sec:heap-Step}
\setCoqFilename{FormalComplexity.TM.LM.StepTM}%
...
\subsection{$\MS{Loop}$}
\label{sec:heap-Loop}
\setCoqFilename{FormalComplexity.TM.LM.HaltingProblem}%
...
For the runtime function $loopSteps$, we need the $step$ and $halt$ functions from Lemma~\ref{lem:heap-red}.  The runtime-function of $\MS{Loop}$ is
