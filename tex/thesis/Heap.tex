...
First, we notice that all types in Definition~\ref{def:Heap}, except program commands ($Com$), are encodable over canonical alphabets, according to
...
First, we derive constructor and deconstructor machines for command.  As the type of commands essentially is the sum of two encodable types ($ACom$
and $\Nat$), we can combine constructors and deconstructors from Section~\ref{sec:constructors-deconstructors}.
\setCoqFilename{ProgrammingTuringMachines.TM.LM.MatchCom}%
\begin{definition}[$\MS{MatchCom}$][MatchCom]
  ...
  \end{alignat*}
\end{definition}
\begin{lemma}[Correctness of $\MS{MatchCom}$][MatchCom_Sem]
  \label{lem:MatchCom_Sem}
  $\MS{MatchCom} \RealiseIn{11} MatchComRel$ with
\begin{lstlisting}[style=semicoqstyle]
$MatchComRel :=$
  $\lambda t~(y, t').~\forall (T:Com).~ t[0] \simeq s \rightarrow \MS{match}~y,T$
...
For the running time function $loopSteps$, we need the $step$ and $halt$ functions from Lemma~\ref{lem:heap-red}.  The running time-function of
