...
\begin{definition}[$\MS{SplitBody}$]
  ...
  \begin{alignat*}{1}
    ...
    & \quad \MS{Else}~~~ \Return{\Nop}{\Some\false}.
  \end{alignat*}
  % Ok version, without the hack for aligning the "=>"s
  % \begin{alignat*}{1}
  %   & \MS{SplitBodyStep} := \\
  %   & \quad \MS{If}~(\LiftTapes{\MS{MatchList}}{\Vector{0;3}}) \\
  %   & \quad \MS{Then}~\Match~(\LiftBoth{\MS{MatchTok}}{RetrList}{\Vector{3}}) \\
  ...
  %   & \quad \phantom{\MS{Then}~(\lambda~[~ \Some{\MS{RET} \Rightarrow}}~\MS{Else}~~~ \Return{\Nop}{\Some\true} \\
  ...
  % \[
  %   \MS{SplitBody} :=
  %   \LiftTapes{\MS{ConstrNil}}{\Vector{2}} \Seq
  %   \LiftBoth{\MS{ConstrO}}{f_{Var}}{\Vector{2}} \Seq
  %   \If{\MS{SplitBodyLoop}}{\Return{(\LiftTapes{\MS{Reset}}{\Vector{2}})}{\true}}{\Return{\Nop}{\false}}
  % \]
  % Constr_nil _ @ [|Fin1|];;
  % Constr_O ⇑ _ @ [|Fin2|];;
  % If (JumpTarget_Loop)
...
\todo{Other definitions, execution protocols, correctness, etc.}
