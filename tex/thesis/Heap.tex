...
\begin{definition}[Semantics of the heap machine]
  ...
  Definition~\ref{def:pow}).  We write $(T,V,H) \nsucc$ if the state is a terminating state.
\end{definition}
...
\begin{lemma}[Correctness of $\MS{Step}$]
  ...
  \begin{alignat*}{1}
    ...
    &\quad \phantom{\MS{if}~y=\None~\MS{then}}~ t'[0] \simeq H' \land t'[1] \simeq V' \land t'[2] \simeq H' \land \left(\forall (i:\Fin_8).~isRight~t'[3+i]\right) \\
    &\quad \MS{else}~ (T,V,H) \nsucc ~\land~ \\
...
\begin{lemma}[Runtime of $\MS{Step}$]
  ...
  \begin{alignat*}{3}
    & StepT &~:=~& \lambda t~k.~\exists T~V~H.~t[0] \simeq H \land t[1] \simeq V \land t[2] \simeq H \land \left(\forall i:\Fin_8.~isRight~t[3+i]\right) \land \\
