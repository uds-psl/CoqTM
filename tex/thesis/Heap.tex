...
An execution protocol for the step machine are depicted in Table~\ref{tab:exec-JumpTarget-RET} and Table~\ref{tab:exec-JumpTarget-VAR}.
...
tapes contain $P'$ and $Q$, and the other tapes are right.  However, $\phi'~k~Q~P = \None$, the correctness relation only commits that the machine
...
$\MS{Loop} := \While~\MS{Step}$.  This machine terminates if and only if the heap state that is encoded on the tape of $\MS{Loop}$ terminates.  This
