...
most one transition.  Primitive machines are the only concrete machines for that we give transition functions $\delta$ explicitly.  They are also the
only concrete machines, for that we fully specify the type of machine states $Q$.  In the following two chapters, we will show how to combine these
machines and build (and verify) complex machines without mentioning $Q$ or $\delta$ again.  All states and transition function will be derived from
these machines.
