...
$\false$) and only have to reason about these labels.  We address the problem that machines are low-level, by introducing abstractions, so that Turing
machines directly manipulate values of arbitrary encodable types.  This makes programming of (and reasoning about) Turing machines comparable to
register machines, but we are not restricted to natural numbers.
