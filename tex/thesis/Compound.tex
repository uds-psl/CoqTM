...
symbol on tape $0$.  It reads the symbol on tape $0$ and writes it to tape $1$.  If the symbol satisfies $h$, then the machine halts.  Else it moves
...
$\MS{CopySymbols}~h$.  However, instead of copying the symbols from one tape to another tape, it ``translates'' the symbols it read, until it read a
symbol that satisfies the boolean predicate $h$.  We leave out the correctness and runtime statements, as they easily can be derived from the
statements above.
...
$\MS{MoveToSymbolStep}~h~f :=$
  Match ($\MS{Read}$)
        ($\lambda (s:\Option(\Sigma)).$ match $s$
           [$\Some{x}$ => (*The head of tape $t[0]$ is on symbol $x$*)
             if $h(x)$
             then $\Return{\bigl(\MS{Write}~(f~x)        \bigr)}{\Some\unit}$ (*break*)
             else $\Return{\bigl(\MS{WriteMove}~(f~x)~R) \bigr)}{\None}$ (*continue*)
           |$\None$ => $\Return{\Nop}{\Some\unit}$ (*No symbol; break*)
           ]).
