...
realises.  We derive the relation $R'$ by applying the correctness lemmas of the combinators, lifts, and concrete machines.  Using the monotonicity
Lemma~\ref{lem:Realise_monotone}, it remains to show $R' \subseteq R$.  Deriving the relation $R'$ is mechanical and does in general not depend on
arbitrary choices (with a few exceptions).  So the main part of the proof is showing $R' \subseteq R$.  The derived relation respects the
``structure'' of the machine.  The ``canonical relation'' of Section~\ref{sec:canonical} is for that reason not suited for the choice of $R'$.  For
example the relation of a sequential composition is the relational composition of two relations.  Note that we do not have to reason about machine
states at all, since the correctness relations are only relations between tapes and labels.  Also note that because the relation $R'$ respects the
structure of $M$, the actual proofs of $R' \subseteq R$ also follows its structure.  As another example, the relation for a conditional is
$(R_1 \at \true \circ R_2) \cup (R_1 \at \false \circ R_3)$.  From that it follows that we do a case-distinction for both branches in the proof.
...
\begin{lemma}[Correctness of $\MS{CopySymbolsStep}$][CopySymbols_Step_Sem]
  ...
  \begin{multline*}
    ...
    \begin{cases}
      ...
      t'[0] = \MS{mv}~R~t[0] \land t'[1]=\MS{doAct}~t[1]~(\Some{x}, R) \land l=\None      & \MS{current}~t[0]=\Some{x} \land \lnot h(x) \\
