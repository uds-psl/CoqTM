...
\usepackage{minted} % Hm? Do I really want to use this package? It depends on Python.
...
\renewcommand{\Tape}{\MS{Tp}}
\renewcommand{\VDash}{\vDash}
\renewcommand{\Vector}[1]{\left[{#1}\right]}
\renewcommand{\tapePointer}[1]{\underset{\uparrow}{#1}}
\renewcommand{\midtape}[3]{#1~\tapePointer{#2}~#3}
...
\begin{frame}
  ...
  \begin{itemize}
  \item $\Tape_\Sigma$: type of tapes
  \item $\MS{TM}_\Sigma^n$: $n$-tape Turing machine with alphabet $\Sigma$
  \item $M : PTM_\Sigma^n(F)$: partitioned machines ($f: Q \to F$ for finite $F$)
  \item $M \VDash R$: Realisation
  \item $M \downarrow T$: Termination
  \end{itemize}
...
\begin{frame}
  ...
  \begin{definition}[Realisation, $M \VDash R$]
    ...
    \begin{multline*}
      M \VDash R :=
      \forall t~t'~q~k.~
      M(t) \triangleright^k (q, t') \rightarrow
      R~t~\left(f(q), t' \right)
    \end{multline*}
  \end{definition}
...
\begin{frame}
  \frametitle{Machine transformations}
...
\begin{frame}
  ...
  Let $M : \MS{PTM}_\Sigma^n$.

  Then $\Uparrow_i M : \MS{PTM}_\Sigma^n$.

  Let $R \subseteq \Tape^m \times F \times \Tape^m$.
  
  Then $\Uparrow_i R \subseteq \Tape^n \times F \times \Tape^n$
  with
  \begin{alignat*}{2}
...
\begin{frame}[fragile]
  \frametitle{Encoding Values on Tapes}
...
\begin{frame}
  ...
  \bibitem{}
    ...
    \newblock{\em Formalizing Turing Machines}%
    \newblock{Lecture Notes in Computer Science, 2012}%
  \bibitem{}
...
\begin{frame}
  \frametitle{Admissible Typing Rules for Combinators}
  \[
    \inferrule{M_1 : \MS{PTM}_\Sigma^n(F_1) \and M_2 : \MS{PTM}_\Sigma^n(F_2)} {M_1;~M_2 : \MS{PTM}_\Sigma^n(F_2)}
  \]
  \[
