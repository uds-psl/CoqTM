...
\begin{frame}
  ...
  \item ($M \downarrow S$: Termination)
  \end{itemize}
...
Class encodable (X : Type) (sig : finType) := {
  ...
  \item
    ...
    \begin{definition}[value-containing, $t \eqsim x$]
      Let $X$ be encodable over $\Sigma$, and $t: \Tape_{\Unit+\Sigma}$.
      \[
        t \eqsim x :=
        \exists r1.~
        t = \midtape{r_1 ~ (\inl \unit)}{(\inr y)}{(\map~\inr~ys)}
      \]
