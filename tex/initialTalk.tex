...
\begin{frame}{Termination of machines}
  \begin{definition}[Termination, $M \downarrow T$]
    ...
    \[
      \forall (t_{in},~n) \in T,~\exists c_{out},~M(t_{in}) \triangleright^n c_{out}
    \]
...
\begin{frame}{Machine transformations}
  ...
  \begin{itemize}
    \item $n$-Lift:
      \onslide<3->{
        \begin{itemize}
          \item Have: injection between tape indexes ($f \from \Fin_n \hookrightarrow \Fin_m$)
          \item Have $n$-tape machine over $\Sigma$
          \item Give $m$-tape machine over $\Sigma$
        \end{itemize}
      }
    \item $\Sigma$-Lift:
      \onslide<4->{
        \begin{itemize}
          \item Have: injection between two alphabets ($f \from \Sigma \hookrightarrow \Tau$)
            with a $\mathbf{default}$ symbol in $\Sigma$
          \item Have $n$-tape machine over $\Sigma$
          \item Give $n$-tape machine over $\Tau$
        \end{itemize}
      }
  \end{itemize}
\end{frame}
