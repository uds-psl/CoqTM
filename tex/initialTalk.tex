...
\begin{frame}{Motivation: Finish the reduction chain}
  ...
  \begin{itemize}
    ...
    \pause\item Goal: build a multi-tape Turing machine that $L$%
...
\begin{frame}{Correctness of machines}
  ...
  \end{definition}
  \pause%
  For \textbf{partioned machines} $M : \setOf{M' : \MS{TM};~ f \from Q_{M'} \to F}$,\\
...
\begin{frame}{Relations}
  ...
  \begin{columns}
    ...
    \begin{column}{0.4\textwidth}
      ...
      \end{alignat*}
      Restriction: {\tiny(for a fixed $a \in F$)}
      \begin{alignat*}{3}
...
\begin{frame}{$n$-Lift}
  Let $f \from \Fin_n \hookrightarrow \Fin_m$ be an injection between tape indexes.
